...
def resolve_default_mesh() -> Path:
    exact_mesh = Path("/home/zdp/CodeField/my_swarm_rl/rl/Indoor-topology-generation/DR_static_mesh_collision.ply")
    if exact_mesh.exists():
        return exact_mesh
    return Path("/home/zdp/CodeField/my_swarm_rl/rl/Indoor-topology-generation/flysite.ply")
...
def load_contact_report(report_path: Path) -> Tuple[List[PairResult], Path]:
    ...
    if raw_points_dir is None:
        ...
        points_dir = (report_dir.parent / raw_points_dir).resolve()

    for raw in payload.get("results", []):
        points_file = Path(raw["points_file"])
        if not points_file.is_absolute():
            direct_candidate = (Path.cwd() / points_file).resolve()
            report_candidate = (report_dir / points_file.name).resolve()
            points_dir_candidate = (points_dir / points_file.name).resolve()
            if direct_candidate.exists():
                points_file = direct_candidate
            elif points_dir_candidate.exists():
                points_file = points_dir_candidate
            else:
                points_file = report_candidate
        results.append(
            PairResult(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--bbox-file",
        type=Path,
        default=Path("/home/zdp/CodeField/my_swarm_rl/rl/sru-navigation-sim/isaaclab_nav_task/navigation/assets/data/Environments/StaticScan/DR_Surface_BBox_Data.txt"),
    )
    parser.add_argument(
        "--report",
        type=Path,
        default=Path("/home/zdp/CodeField/my_swarm_rl/rl/logs/fixed_height_validation/fixed_height_contact_full.json"),
    )
    parser.add_argument(
        "--safe-points",
        type=Path,
